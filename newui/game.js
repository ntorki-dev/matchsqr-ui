...
// v10: Show guidance when game is running and there is no active turn
    ...
    if (s.status==='ended'){
      ...
      try{
        ...
        (function(){
          const entries = Game.seatOrder(Game.state.host_user_id, Game.state.participants||[]);
          const mappedParticipants = entries.map(e=>({ ...e.p, seat_index: e.idx }));
          const stateForSummary = { ...Game.state, participants: mappedParticipants };
          Summary.mount({
            container: main,
            state: stateForSummary,
            selectedSeat: Game.ui.summarySelectedSeat,
            code: Game.code,
            myPid,
            isHost
          });

...
// Ensures page content never hides behind the fixed footer without changing existing logic/exports.
(function () {
  function applyPadding(footer) {
    const h = (footer && footer.offsetHeight) ? footer.offsetHeight : 0;
    document.documentElement.style.setProperty('--footer-h', h + 'px');
    if (document.body) document.body.style.paddingBottom = 'var(--footer-h)';
  }

  function bindResize(footer){
    function update(){ applyPadding(footer); }
  ...
  function init(){
    ...
    if (footer){ bindResize(footer); return; }
    // If footer not present yet, observe DOM until it appears
    try {
      const mo = new MutationObserver((mutations, obs) => {
        const f = document.querySelector('.site-footer');
        if (f){ bindResize(f); obs.disconnect(); }
      });
      mo.observe(document.documentElement, { childList: true, subtree: true });
    } catch(_) {}
  }

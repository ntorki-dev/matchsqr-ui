...
async function attachGuestIfPending(sb) {
  try {
    ...
    toast('Your previous session was attached to your account.'); return true;
...
function showResendControl({ container, email }) {
  ...
  if (!row) {
    ...
    row.id = 'resendRow';
    row.className = 'grid';
    row.innerHTML = `<button id="resendBtn" class="btn">Resend confirmation email</button><span id="resendMsg" class="help"></span>`;
  ...
  let t=null, left=0;
  const cooldown=(secs)=>{ left=secs; btn.disabled=true;
    const tick=()=>{ if(left<=0){ btn.disabled=false; msg.textContent=''; clearInterval(t); t=null; return;} msg.textContent=`Email sent. You can resend in ${left}s.`; left--; };
    tick(); t=setInterval(tick,1000);
  };
  btn.onclick = async () => {
    try{ const sb = await ensureClient(); await sb.auth.resend({ type:'signup', email, options:{ emailRedirectTo: location.origin + '/#/account' } }); cooldown(60); }
...
export async function render(ctx){
  ...
  if (!user) {
    ...
    location.hash = '#/login';
    app.innerHTML = `<div class="offline-banner"></div><div class="container"><div class="host-wrap"><h2>Account</h2><p>You need to log in to view your account.</p><p><a class="btn" href="#/login?next=%2F#%2Faccount">Go to Login</a></p></div></div>`;
    await renderHeader(); ensureDebugTray(); return;
  }

  const dbName = await getProfileName(user.id);
  const name = dbName || user?.user_metadata?.name || (user?.email ? user.email.split('@')[0] : 'Account');
  app.innerHTML = `<div class="offline-banner"></div><div class="container"><div class="host-wrap"><h2>Welcome, ${name}</h2><div class="grid"><button id="logoutBtn" class="ghost">Logout</button></div></div></div>`;
  await renderHeader(); ensureDebugTray();
  try { const sb = await ensureClient(); await attachGuestIfPending(sb); } catch {}
  $('#logoutBtn').onclick = async () => { try{ const sb=await ensureClient(); await sb.auth.signOut(); }catch{} try{ localStorage.removeItem('ms_lastKnownUser'); localStorage.removeItem('remember_me'); sessionStorage.removeItem('remember_me'); sessionStorage.removeItem('__redirect_after_login'); }catch{} try{ await renderHeader(); }catch{} location.hash='#/login'; };
}

async function renderLogin(){
  const app=document.getElementById('app');
  const redirectTo = sessionStorage.getItem('__redirect_after_login') || '#/';
  app.innerHTML = `<div class="offline-banner"></div><div class="container"><div class="host-wrap"><h2>Login</h2><div id="loginCard" class="grid"><input id="email" class="input" placeholder="Email" type="email" autocomplete="email"><input id="password" class="input" placeholder="Password" type="password" autocomplete="current-password"><label class="help"><input id="remember" type="checkbox"> Remember me</label><button id="loginBtn" class="btn">Login</button><div class="inline-actions"><a class="help" href="#/account">Account</a><a class="help" href="#/help">Help</a></div></div></div></div>`;
  await renderHeader(); ensureDebugTray();
  $('#loginBtn').onclick=async()=>{
    const email = $('#email').value.trim(); const password = $('#password').value;
    try{ const sb = await ensureClient(); const { error } = await sb.auth.signInWithPassword({ email, password });
      if (error) { const msg = (''+(error?.message||'')).toLowerCase(); if(msg.includes('confirm')||msg.includes('verified')) showResendControl({ container: $('#loginCard'), email }); throw error; }
      const remember = !!$('#remember').checked; (remember?localStorage:sessionStorage).setItem('remember_me', JSON.stringify(remember));
      await attachGuestIfPending(sb); location.hash = redirectTo; sessionStorage.removeItem('__redirect_after_login');
...
async function renderRegister(){
  ...
  function computeAge(isoDate){ try{ const d=new Date(isoDate), t=new Date(); let a=t.getFullYear()-d.getFullYear(); const m=t.getMonth()-d.getMonth(); if(m<0||(m===0&&t.getDate()<d.getDate())) a--; return a; }catch{ return NaN; } }
  ...
  $('#reg_submit').onclick = async () => {
    ...
    try{
      ...
      const { data, error } = await sb.auth.signUp({ email, password, options: { data: { name, birthdate: dobISO, gender }, emailRedirectTo: location.origin + '/#/account' } });
      if (error) throw error;
      $('#reg_submit').disabled = true;
      const needsConfirm = (!!data?.user && !data?.session);
      if (needsConfirm) { $('#reg_msg').textContent = 'Check your email to confirm your account.'; showResendControl({ container: $('#resendWrap'), email }); }
      else { $('#reg_msg').textContent = 'Registration successful. Redirecting...'; await attachGuestIfPending(sb); location.hash = redirectAfter; }

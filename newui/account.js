...
/** Resend confirmation controls (compact, centered) */
...
/** Dedicated confirmation screen (replaces the form) */
    ...
    try{
      ...
      if (error) {
        ...
        if (msg.includes('confirm') || msg.includes('verified')) {
          // mount compact resend below the form
          mountResendControls($('#loginCard'), email);
        }
        throw error;
      }
...
async function renderRegister(){
  ...
  app.innerHTML = `
    ...
    <div class="container">
      ...
      <div class="host-wrap">
        ...
        <!-- Consent: add comfortable top margin and line spacing -->
  ...
  $('#reg_submit').onclick = async () => {
    ...
    try{
      ...
      // If confirmation is required, switch to dedicated confirmation screen
      const needsConfirm = (!!data?.user && !data?.session);
      if (needsConfirm) {
        await renderConfirmEmailScreen(email);
        return;
      }

      // If confirmation not required, attach guest (if any) and go to account
      await attachGuestIfPending(sb);

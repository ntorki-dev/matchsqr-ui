...
try{ window.els = els; }catch(_){}
  ...
  try{
    const sess = (state && (state.session || null));
...
(function (global, d) {
  ...
  async function renderHostLobby(){
    ...
    if (!(global.MatchSquareApp && MatchSquareApp.screens && MatchSquareApp.screens.hostLobby)){
      try{ await loadScriptOnce('pages/hostlobby.js'); }catch(e1){ await loadScriptOnce('/newui/pages/hostlobby.js'); }
    }
